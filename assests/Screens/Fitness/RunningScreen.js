...
const COLORS = {
  primary: '#23238E',
  secondary: '#6B6BE8', // Specific color for Running
  accent: '#FFB800',
  white: '#FFFFFF',
  text: '#1A1A2E',
...
export default function RunningScreen({ navigation, route }) {
  const activityType = route?.params?.activeTab || 'Running';
  ...
  useEffect(() => {
    if (isTracking) {
      timerInterval.current = setInterval(() => {
        ...
        // Simulate progress
        setDistance((prev) => prev + 0.003); // ~11km/h
        setCalories((prev) => prev + 0.08);
        setPace(5.5 + (Math.random() * 0.5)); // min/km
      }, 1000);
        ...
        <View style={styles.header}>
          ...
          <View style={styles.headerTextContainer}>
            ...
            <View style={styles.statusBadge}>
              ...
              <Text style={styles.statusText}>{isTracking ? 'TRACKING' : 'READY'}</Text>
            </View>
          </View>
        ...
        >
          ...
          <View style={styles.secondaryStats}>
            ...
            <View style={styles.statItem}>
              ...
              <Text style={styles.statValue}>{pace.toFixed(1)} <Text style={styles.statUnit}>/km</Text></Text>
            </View>
            <View style={styles.statItem}>
          ...
          <View style={styles.controlsRow}>
            {isTracking ? (
              <>
                ...
                <TouchableOpacity style={[styles.mainControl, { backgroundColor: COLORS.secondary }]} onPress={toggleTracking}>
                  <Ionicons name="pause" size={moderateScale(32)} color={COLORS.white} />
                </TouchableOpacity>
              </>
            ) : (
              <TouchableOpacity
                style={[styles.mainControl, { width: wp(70), backgroundColor: COLORS.secondary }]}
...
const styles = StyleSheet.create({
  ...
  markerOutline: {
    ...
    height: scale(24),
    borderRadius: scale(12),
    backgroundColor: 'rgba(107, 107, 232, 0.2)',
    alignItems: 'center',
    justifyContent: 'center',
    borderWidth: 2,
